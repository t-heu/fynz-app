...
export function FormConta({ open, onClose, editando, onSaved }: Props) {
  ...
  function salvarConta() {
    ...
    // 1. Crie uma cópia profunda do objeto de dados
    const novosDados = { ...dados }

    if (editando) {
      // 2. AQUI ESTÁ O PULO DO GATO: Crie um NOVO array para as contas
      const novasContas = [...novosDados.contas] 
      const idx = novasContas.findIndex((x: any) => x.id === editando.id)
      
      // Substitui no novo array
      novasContas[idx] = novaConta
      
      // Atribui o novo array ao objeto principal
      novosDados.contas = novasContas

      // O mesmo vale para os lançamentos, se for alterar:
      ...
      // Para adicionar, também crie um novo array
      novosDados.contas = [...novosDados.contas, novaConta]
      // ... mesma lógica para lançamentos se necessário
    }
    ...
    <Modal visible={open} animationType="slide" onRequestClose={onClose}>
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>{editando ? "Gerenciar Conta" : "Nova Conta"}</Text>
        ...
        {/* Sub-tela: Seleção de Logo */}
        <Modal visible={selecionandoLogo} animationType="slide" onRequestClose={() => setSelecionandoLogo(false)}>
          <View style={styles.container}>
            <View style={styles.header}>
              <TouchableOpacity onPress={() => setSelecionandoLogo(false)}>
                <ChevronLeft size={24} color={currentTheme.foreground} />
              </TouchableOpacity>
              <Text style={styles.title}>Selecionar Logo</Text>
              <View style={{ width: 24 }} />
            </View>
